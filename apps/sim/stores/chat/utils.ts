...
const calculateDefaultPosition = (): ChatPosition => {
  ...
  // Get sidebar width for RTL positioning
  const sidebarWidth = Number.parseInt(
    getComputedStyle(document.documentElement).getPropertyValue('--sidebar-width') || '0'
  )
  const isRTL = document.documentElement.dir === 'rtl'

  // Position in top-end of canvas, 32px from top and 32px from the panel edge
  const x = isRTL
    ? sidebarWidth + 32
    : window.innerWidth - panelWidth - 32 - DEFAULT_WIDTH
  const y = 32
...
export const constrainChatPosition = (
  ...
  // Calculate bounds (swap sidebar/panel sides in RTL)
  const isRTL = document.documentElement.dir === 'rtl'
  const minX = isRTL ? panelWidth : sidebarWidth
  const maxX = isRTL ? window.innerWidth - sidebarWidth - width : window.innerWidth - panelWidth - width
  const minY = 0
