          ...
          <ModalHeader>
            <div className='flex items-center gap-[10px]'>
              ...
              <h2 className='font-semibold text-[16px]'>
                {isSingleRow ? t('tables.row.deleteRow') : `${t('common.delete')} ${deleteCount} ${t('tables.deleteModal.rows')}`}
              </h2>
            </div>
          </ModalHeader>
          <ModalBody>
            <div className='flex flex-col gap-[16px]'>
              ...
              <p className='text-[14px] text-[var(--text-secondary)]'>
                {isSingleRow ? t('tables.row.deleteConfirm') : t('tables.row.deleteMultiConfirm')}
              </p>
            </div>
          </ModalBody>
      ...
      <ModalContent className='w-[600px]'>
        <ModalHeader>
          <div className='flex flex-col gap-[4px]'>
            <h2 className='font-semibold text-[16px]'>{isAddMode ? t('tables.row.addRow') : t('tables.row.editRow')}</h2>
            <p className='font-normal text-[13px] text-[var(--text-tertiary)]'>
              {isAddMode ? t('tables.row.fillValues') : t('tables.row.updateValues')} {table?.name ?? 'table'}
            </p>
          </div>
        </ModalHeader>
          ...
          >
            {isSubmitting
              ? isAddMode
                ...
                : t('common.updating')
              : isAddMode
                ? t('tables.row.addRowButton')
                : t('tables.row.saveChanges')}
          </Button>
        </ModalFooter>
      </ModalContent>
    </Modal>
  ...
  return (
    <div className='flex flex-col gap-[8px]'>
      ...
      <div className='text-[12px] text-[var(--text-tertiary)]'>
        {t('tables.schema.type')}: {column.type}
        {!column.required && ` (${t('common.optional').toLowerCase()})`}
      </div>
    </div>
  )
}
