    ...
    <>
        ...
        <div className='group flex h-full flex-col gap-[12px] rounded-[4px] bg-[var(--surface-3)] px-[8px] py-[6px] transition-colors hover:bg-[var(--surface-4)] dark:bg-[var(--surface-4)] dark:hover:bg-[var(--surface-5)]'>
          ...
          <div className='flex flex-1 flex-col gap-[8px]'>
            <div className='flex items-center justify-between'>
              <div className='flex items-center gap-[12px] text-[12px] text-[var(--text-tertiary)]'>
                <span className='flex items-center gap-[4px]'>
                  ...
                  {columnCount} {columnCount === 1 ? t('tables.card.col') : t('tables.card.cols')}
                </span>
                <span className='flex items-center gap-[4px]'>
                  <Rows3 className='h-[12px] w-[12px]' />
                  {table.rowCount} {table.rowCount === 1 ? t('tables.card.row') : t('tables.card.rows')}
                </span>
              </div>
              <Tooltip.Root>
            ...
            <p className='line-clamp-2 h-[36px] text-[12px] text-[var(--text-tertiary)] leading-[18px]'>
              {table.description || t('tables.noDescription')}
            </p>
          </div>
        </div>
      </div>
      ...
      <Modal open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <ModalContent className='w-[400px]'>
          <ModalHeader>{t('tables.deleteModal.title')}</ModalHeader>
          <ModalBody>
            <p className='text-[12px] text-[var(--text-secondary)]'>
              {t('tables.deleteModal.confirmText')}{' '}
              <span className='font-medium text-[var(--text-primary)]'>{table.name}</span>?{' '}
              {t('tables.deleteModal.rowCount')} {table.rowCount} {t('tables.deleteModal.rows')}.{' '}
