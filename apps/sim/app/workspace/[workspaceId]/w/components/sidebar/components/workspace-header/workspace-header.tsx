    ...
    <div className={`flex items-center gap-[8px] ${isCollapsed ? '' : 'min-w-0 justify-between'}`}>
          ...
          >
            <PopoverTrigger asChild>
              <button
                ...
                title={activeWorkspace?.name || t('common.loading')}
                onContextMenu={(e) => {
            ...
            >
              {isWorkspacesLoading ? (
                <PopoverItem disabled>
                  <span>{t('workspace.loadingWorkspaces')}</span>
                </PopoverItem>
              ) : (
                <>
                  <div className='relative flex items-center justify-between'>
                    <PopoverSection>{t('workspace.workspaces')}</PopoverSection>
                      ...
                      <Tooltip.Root>
                        <Tooltip.Trigger asChild>
                          <Button
                            variant='ghost'
                            type='button'
                            aria-label={t('workspace.importWorkspace')}
                            className='!p-[3px]'
                            onClick={(e) => {
                              e.stopPropagation()
                              onImportWorkspace()
      ...
      <Modal open={isLeaveModalOpen} onOpenChange={() => setIsLeaveModalOpen(false)}>
        <ModalContent size='sm'>
          <ModalHeader>{t('workspace.leaveWorkspace')}</ModalHeader>
          <ModalBody>
          ...
          <ModalFooter>
            ...
            <Button variant='destructive' onClick={handleLeaveWorkspace} disabled={isLeaving}>
              {isLeaving ? t('common.leaving') : t('workspace.leaveWorkspace')}
            </Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
