  ...
  if (isSaving) {
    return (
      <div className='space-y-[12px]'>
        <h3 className='font-medium text-[14px] text-[var(--text-primary)]'>{t('workspace.invite.memberPermissions')}</h3>
        <div className='rounded-[8px] border border-[var(--border-1)] bg-[var(--surface-3)]'>
          <div className='flex items-center justify-center py-[48px]'>
            <div className='flex items-center gap-[8px] text-[var(--text-secondary)]'>
              ...
              <span className='font-medium text-[13px]'>{t('workspace.invite.savingPermissions')}</span>
            </div>
          </div>
        </div>
        <p className='flex min-h-[2rem] items-start text-[12px] text-[var(--text-tertiary)]'>
          {t('workspace.invite.waitPermissions')}
        </p>
      </div>
    )
  }
                    ...
                    {isPendingInvitation && (
                      <Badge variant='default' className='gap-[4px] text-[12px]'>
                        ...
                        resendingInvitationIds[user.invitationId] ? (
                          <span>{t('workspace.invite.sending')}</span>
                        ) : resentInvitationIds &&
                          user.invitationId &&
                          resentInvitationIds[user.invitationId] ? (
                          <span>{t('workspace.invite.resent')}</span>
                        ) : (
                          <span>{t('workspace.invite.sent')}</span>
                        )}
                      </Badge>
                    )}
                    {hasChanges && (
                          ...
                          <Tooltip.Trigger asChild>
                            <span className='inline-flex'>
                              ...
                              >
                                ...
                                <span className='sr-only'>{t('workspace.invite.resendInvite')}</span>
                              </Button>
                            </span>
                          </Tooltip.Trigger>
                          <Tooltip.Content>
                            <p>
                              {resendCooldowns?.[user.invitationId!]
                                ? `${t('workspace.invite.resendIn')} ${resendCooldowns[user.invitationId!]}s`
                                : t('workspace.invite.resendInvite')}
                            </p>
                          </Tooltip.Content>
                        </Tooltip.Root>
                      )}
                        ...
                        <Tooltip.Trigger asChild>
                          ...
                          >
                            ...
                            <span className='sr-only'>
                              {isPendingInvitation ? t('workspace.invite.revokeInvite') : t('workspace.invite.removeMemberAction')}
                            </span>
                          </Button>
                        </Tooltip.Trigger>
                        <Tooltip.Content>
                          <p>{isPendingInvitation ? t('workspace.invite.revokeInvite') : t('workspace.invite.removeMemberAction')}</p>
                        </Tooltip.Content>
                      </Tooltip.Root>
                    )}
                  </div>
