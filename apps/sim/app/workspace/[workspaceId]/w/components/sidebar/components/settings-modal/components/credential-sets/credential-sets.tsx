    ...
    <>
      <div className='flex h-full flex-col gap-[16px]'>
        <div className='flex items-center gap-[8px]'>
          <div className='flex flex-1 items-center gap-[8px] rounded-[8px] border border-[var(--border)] bg-transparent px-[8px] py-[5px] transition-colors duration-100 dark:bg-[var(--surface-4)] dark:hover:border-[var(--border-1)] dark:hover:bg-[var(--surface-5)]'>
            ...
            />
            <BaseInput
              placeholder={t('settings.credentialSets.searchPlaceholder')}
              value={searchTerm}
      ...
      <Modal open={showCreateModal} onOpenChange={handleCloseCreateModal}>
        <ModalContent size='sm'>
          <ModalHeader>{t('settings.credentialSets.createTitle')}</ModalHeader>
          <ModalBody>
            <div className='flex flex-col gap-[12px]'>
              <div className='flex flex-col gap-[4px]'>
                <Label>{t('settings.credentialSets.nameLabel')}</Label>
                <Input
                  value={newSetName}
                  onChange={(e) => {
                  ...
                  placeholder={t('settings.credentialSets.namePlaceholder')}
                />
              </div>
              <div className='flex flex-col gap-[4px]'>
                <Label>{t('settings.credentialSets.descriptionLabel')}</Label>
                <Input
                  value={newSetDescription}
                  onChange={(e) => setNewSetDescription(e.target.value)}
                  placeholder={t('settings.credentialSets.descriptionPlaceholder')}
                />
              </div>
              <div className='flex flex-col gap-[4px]'>
                <Label>{t('settings.credentialSets.emailProvider')}</Label>
          ...
          <ModalFooter>
            ...
            >
              {createCredentialSet.isPending ? t('common.creating') : t('common.create')}
            </Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
      ...
      <Modal open={!!leavingMembership} onOpenChange={() => setLeavingMembership(null)}>
        <ModalContent size='sm'>
          <ModalHeader>{t('settings.credentialSets.leaveTitle')}</ModalHeader>
          <ModalBody>
      ...
      <Modal open={!!deletingSet} onOpenChange={() => setDeletingSet(null)}>
        <ModalContent size='sm'>
          <ModalHeader>{t('settings.credentialSets.deleteTitle')}</ModalHeader>
          <ModalBody>
            <p className='text-[12px] text-[var(--text-secondary)]'>
          ...
          <ModalFooter>
            ...
            >
              {deleteCredentialSet.isPending ? t('common.deleting') : t('common.delete')}
            </Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
