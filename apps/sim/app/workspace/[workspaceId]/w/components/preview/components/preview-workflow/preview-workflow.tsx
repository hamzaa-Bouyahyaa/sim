...
const previewNodeTypes: NodeTypes = {
  ...
  minimalPill: PreviewBlock,
  conditionalNode: PreviewBlock,
  databaseSchema: PreviewBlock,
  richCard: PreviewBlock,
  statusNode: PreviewBlock,
  circleNode: PreviewBlock,
}
...
export function PreviewWorkflow({
  ...
  const nodes: Node[] = useMemo(() => {
    ...
    Object.entries(workflowState.blocks || {}).forEach(([blockId, block]) => {
      ...
      const blockCfg = getBlock(block.type)
      const nodeType = block.type === 'note' ? 'noteBlock' : (blockCfg?.nodeType ?? 'workflowBlock')
