...
export const getDisplayValue = (value: unknown): string => {
  ...
  if (isTagFilterArray(parsedValue)) {
    ...
    if (validFilters.length === 2)
      return `${validFilters[0].tagName}, ${validFilters[1].tagName}`;
  ...
  if (isDocumentTagArray(parsedValue)) {
    ...
    if (validTags.length === 2)
      return `${validTags[0].tagName}, ${validTags[1].tagName}`;
  ...
  if (isFilterConditionArray(parsedValue)) {
    ...
    if (validConditions.length === 1)
      return formatCondition(validConditions[0]);
  ...
  if (isSortConditionArray(parsedValue)) {
    ...
    if (validConditions.length === 0) return "-";
    const formatSort = (c: SortRule) =>
      `${c.column} ${c.direction === "desc" ? "↓" : "↑"}`;
  ...
  if (isTableRowArray(parsedValue)) {
    ...
    if (nonEmptyRows.length === 1) {
      ...
      const cellEntries = Object.entries(firstRow.cells).filter(([, val]) =>
        val?.trim(),
      );
  ...
  if (isFieldFormatArray(parsedValue)) {
    ...
    if (namedFields.length === 2)
      return `${namedFields[0].name}, ${namedFields[1].name}`;
  ...
  if (Array.isArray(parsedValue)) {
    ...
    const getItemDisplayValue = (item: unknown): string => {
      if (typeof item === "object" && item !== null) {
        ...
        return String(
          obj.title || obj.name || obj.label || obj.id || JSON.stringify(item),
        );
      }
      return String(item);
    };
...
const areSubBlockRowPropsEqual = (
  ...
  const prevValue = subBlockId
    ? prevProps.allSubBlockValues?.[subBlockId]?.value
    : undefined;
  const nextValue = subBlockId
    ? nextProps.allSubBlockValues?.[subBlockId]?.value
    : undefined;
...
const SubBlockRow = memo(function SubBlockRow({
  ...
  const credentialSourceId =
    subBlock?.type === "oauth-input" && typeof rawValue === "string"
      ? rawValue
      : undefined;
  ...
  const mcpServerDisplayName = useMemo(() => {
    if (
      subBlock?.type !== "mcp-server-selector" ||
  ...
  const webhookUrlDisplayValue = useMemo(() => {
    ...
    const triggerPath = allSubBlockValues?.triggerPath?.value as
      | string
      | undefined;
    return triggerPath
  ...
  const variablesDisplayValue = useMemo(() => {
    if (
      ...
      !isVariableAssignmentsArray(rawValue)
    ) {
      return null;
    }
    ...
    const names = rawValue
      .map((a) => {
        if (a.variableId) {
          const variable = variablesArray.find(
            (v: any) => v.id === a.variableId,
          );
      ...
      .map((tool: any) => {
        ...
        if (tool.type === "custom-tool" && tool.customToolId) {
          const customTool = customTools.find(
            (t) => t.id === tool.customToolId,
          );
          if (customTool?.title) return customTool.title;
          if (customTool?.schema?.function?.name)
            return customTool.schema.function.name;
        }
  ...
  const skillsDisplayValue = useMemo(() => {
    ...
    const skillNames = rawValue
      .map((skill: StoredSkill) => {
        ...
        if (skill.skillId) {
          const foundSkill = workspaceSkills.find(
  ...
  const isSelectorType =
    subBlock?.type && SELECTOR_TYPES_HYDRATION_REQUIRED.includes(subBlock.type);
  const hydratedName =
    credentialName ||
    dropdownLabel ||
    ...
    selectorDisplayName;
  const displayValue =
    maskedValue || hydratedName || (isSelectorType && value ? "-" : value);

  return (
    <div className="flex items-center gap-[8px]">
      <span
        className="min-w-0 truncate text-[15px] font-medium text-[var(--text-tertiary)] capitalize"
        title={title}
      >
        {title}
      </span>
      {displayValue !== undefined && (
        <span
          className={cn(
            "flex-1 truncate text-right text-[15px] text-[var(--text-primary)]",
...
export const WorkflowBlock = memo(function WorkflowBlock({
  ...
  const { childWorkflowId, childIsDeployed, childNeedsRedeploy } =
    useChildWorkflow(id, type, data.isPreview ?? false, data.subBlockValues);

  const { mutate: deployChildWorkflow, isPending: isDeploying } =
  ...
  const isWebhookTriggerBlock =
    type === "webhook" || type === "generic_webhook";
  ...
  const blockSubBlockValues = useStoreWithEqualityFn(
    ...
    useCallback(
      (state) => {
        ...
        return (
          state.workflowValues[activeWorkflowId]?.[id] ?? EMPTY_SUBBLOCK_VALUES
        );
      },
      [activeWorkflowId, id],
    ),
  ...
  const canonicalIndex = useMemo(
    () => buildCanonicalIndex(config.subBlocks),
    [config.subBlocks],
  );
  ...
  const subBlockRowsData = useMemo(() => {
    ...
    const rawValues = Object.entries(stateToUse).reduce<
    ...
    const effectiveAdvanced = userPermissions.canEdit
      ? displayAdvancedMode
      : displayAdvancedMode ||
        hasAdvancedValues(config.subBlocks, rawValues, canonicalIndex);
  ...
  const effectiveAdvanced = useMemo(() => {
    const rawValues = Object.entries(subBlockState).reduce<
      Record<string, unknown>
    >((acc, [key, entry]) => {
      acc[key] = entry?.value;
      return acc;
  ...
  const hasContentBelowHeader =
    subBlockRows.length > 0 || shouldShowDefaultHandles;
  ...
  const getHandleClasses = (
    ...
    const baseClasses =
      "!z-[10] !cursor-crosshair !border-none !transition-[colors] !duration-150";
    const colorClasses = isError
      ? "!bg-[var(--text-error)]"
      : "!bg-[var(--workflow-edge)]";

    const positionClasses = {
      left: "!left-[-6px] !h-[12px] !w-[12px] !rounded-full hover:!left-[-8px] hover:!h-[16px] hover:!w-[16px]",
      right:
        "!right-[-6px] !h-[12px] !w-[12px] !rounded-full hover:!right-[-8px] hover:!h-[16px] hover:!w-[16px]",
      top: "!top-[-6px] !h-[12px] !w-[12px] !rounded-full hover:!top-[-8px] hover:!h-[16px] hover:!w-[16px]",
      bottom:
        "!bottom-[-6px] !h-[12px] !w-[12px] !rounded-full hover:!bottom-[-8px] hover:!h-[16px] hover:!w-[16px]",
    };
  ...
  const conditionRows = useMemo(() => {
    ...
    const raw =
      typeof conditionsValue === "string" ? conditionsValue : undefined;
    ...
    try {
        ...
        if (Array.isArray(parsed)) {
          return parsed.map((item: unknown, index: number) => {
            ...
            return {
              ...
              title,
              value:
                typeof conditionItem?.value === "string"
                  ? conditionItem.value
                  : "",
            };
          });
      ...
      logger.warn("Failed to parse condition subblock value", {
        error,
        blockId: id,
      });
    }
  ...
  const routerRows = useMemo(() => {
    ...
    try {
      ...
      logger.warn("Failed to parse router routes value", {
        error,
        blockId: id,
      });
    }
  ...
  useBlockDimensions({
    ...
    calculateDimensions: () => {
      ...
      if (type === "condition") {
        ...
        const subblockRowCount = subBlockRows.reduce(
  ...
  const showWebhookIndicator =
    (isStarterBlock || isWebhookTriggerBlock) && isWebhookConfigured;
  ...
  return (
    <div className="group relative">
      <div
        ...
        onClick={handleClick}
        className={cn(
          "workflow-drag-handle relative z-[20] w-[350px] cursor-grab select-none rounded-[4px] border-2 border-[var(--border-1)] [&:active]:cursor-grabbing",
        )}
        style={{
      ...
      >
        ...
        {shouldShowDefaultHandles && (
          <Handle
            ...
            style={getHandleStyle(
              horizontalHandles ? "horizontal" : "vertical",
            )}
            data-nodeid={id}
        ...
        >
          <div className="relative z-10 flex min-w-0 flex-1 items-center gap-[10px]">
            ...
            >
              <config.icon className="h-[18px] w-[18px] text-white" />
            </div>
            <span
              className={cn(
                "truncate font-semibold text-[18px]",
                !isEnabled &&
            ...
            <>
              <Handle
                ...
                className={getHandleClasses(
                  horizontalHandles ? "right" : "bottom",
                )}
                style={getHandleStyle(
                  horizontalHandles ? "horizontal" : "vertical",
                )}
