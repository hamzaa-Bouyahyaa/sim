                ...
                >
                  ...
                  <PopoverContent align='end' sideOffset={4} minWidth={160} maxWidth={200} border>
                    ...
                    <PopoverItem onClick={() => handleOpenDescriptionModal(v.version)}>
                      ...
                      <span>{v.description ? t('common.edit') + ' ' + t('common.description').toLowerCase() : t('common.add') + ' ' + t('common.description').toLowerCase()}</span>
                    </PopoverItem>
                    {!v.isActive && (
                      <PopoverItem onClick={() => handlePromote(v.version)}>
                        <RotateCcw className='h-3 w-3' />
                        <span>{t('deploy.modal.general.promoteToLive')}</span>
                      </PopoverItem>
                    )}
                    <PopoverItem onClick={() => handleLoadDeployment(v.version)}>
                      <SendToBack className='h-3 w-3' />
                      <span>{t('deploy.modal.general.loadButton')}</span>
                    </PopoverItem>
                  </PopoverContent>
                </Popover>
              </div>
