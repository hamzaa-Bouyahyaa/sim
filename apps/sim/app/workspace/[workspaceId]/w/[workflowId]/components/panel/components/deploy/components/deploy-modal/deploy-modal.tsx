    ...
    <>
      <Modal open={open} onOpenChange={handleCloseModal}>
        <ModalContent size='lg' className='h-[76vh]'>
          <ModalHeader>{t('deploy.modal.title')}</ModalHeader>
          ...
          >
            <ModalTabsList activeValue={activeTab}>
              <ModalTabsTrigger value='general'>{t('deploy.modal.tabs.general')}</ModalTabsTrigger>
              {!permissionConfig.hideDeployApi && (
                <ModalTabsTrigger value='api'>{t('deploy.modal.tabs.api')}</ModalTabsTrigger>
              )}
              {!permissionConfig.hideDeployMcp && (
                <ModalTabsTrigger value='mcp'>{t('deploy.modal.tabs.mcp')}</ModalTabsTrigger>
              )}
              {!permissionConfig.hideDeployA2a && (
                <ModalTabsTrigger value='a2a'>{t('deploy.modal.tabs.a2a')}</ModalTabsTrigger>
              )}
              {!permissionConfig.hideDeployChatbot && (
                <ModalTabsTrigger value='chat'>{t('deploy.modal.tabs.chat')}</ModalTabsTrigger>
              )}
              {/* <ModalTabsTrigger value='form'>Form</ModalTabsTrigger> */}
              {!permissionConfig.hideDeployTemplate && (
                <ModalTabsTrigger value='template'>{t('deploy.modal.tabs.template')}</ModalTabsTrigger>
              )}
            </ModalTabsList>
            ...
            <ModalFooter className='items-center justify-between'>
              ...
              <div className='flex items-center gap-2'>
                {chatExists && (
                  ...
                  >
                    {t('deploy.modal.chat.deleteButton')}
                  </Button>
                )}
                <Button
                  type='button'
                ...
                >
                  {chatSuccess
                    ? chatExists
                      ? t('deploy.modal.chat.updated')
                      : 'Launched'
                    : chatSubmitting
                      ? chatExists
                        ? t('common.updating')
                        : t('deploy.modal.chat.launching')
                      : chatExists
                        ? t('deploy.modal.chat.updateChat')
                        : t('deploy.modal.chat.launchChat')}
                </Button>
              </div>
            </ModalFooter>
          )}
          {activeTab === 'mcp' && isDeployed && hasMcpServers && (
            <ModalFooter className='items-center justify-between'>
              ...
              <div className='flex items-center gap-2'>
                ...
                >
                  {mcpToolSubmitting ? t('common.saving') : t('deploy.modal.mcp.saveTool')}
                </Button>
              </div>
            </ModalFooter>
          )}
            ...
            <ModalFooter className='items-center justify-between'>
              ...
              <div className='flex items-center gap-2'>
                {hasExistingTemplate && (
                  ...
                  >
                    {t('deploy.modal.template.deleteTemplate')}
                  </Button>
                )}
                <Button
                  type='button'
                ...
                >
                  {templateSubmitting
                    ? hasExistingTemplate
                      ? t('deploy.modal.template.updating')
                      : t('deploy.modal.template.publishing')
                    : hasExistingTemplate
                      ? t('deploy.modal.template.updateTemplate')
                      : t('deploy.modal.template.publishTemplate')}
                </Button>
              </div>
            </ModalFooter>
          )}
            ...
            <ModalFooter className='items-center justify-between'>
              ...
              {hasA2aAgent ? (
                isA2aPublished ? (
                  <Badge variant={a2aNeedsRepublish ? 'amber' : 'green'} size='lg' dot>
                    {a2aNeedsRepublish ? t('deploy.updateDeployment') : t('common.live')}
                  </Badge>
                ) : (
                  <Badge variant='red' size='lg' dot>
                    {t('deploy.modal.a2a.unpublished')}
                  </Badge>
                )
              ) : (
                <div />
              ...
              <div className='flex items-center gap-2'>
                ...
                {!hasA2aAgent && (
                  ...
                  >
                    {a2aSubmitting ? t('deploy.modal.template.publishing') : t('deploy.modal.a2a.publishAgent')}
                  </Button>
                )}
                ...
                {hasA2aAgent && isA2aPublished && (
                  <>
                    ...
                    >
                      {a2aSubmitting ? t('common.updating') : t('common.update')}
                    </Button>
                  </>
                )}
      ...
      <Modal open={showUndeployConfirm} onOpenChange={setShowUndeployConfirm}>
        <ModalContent size='sm'>
          <ModalHeader>{t('deploy.modal.undeploy.title')}</ModalHeader>
          <ModalBody>
            <p className='text-[12px] text-[var(--text-secondary)]'>
              {t('deploy.modal.undeploy.confirm')}{' '}
              <span className='text-[var(--text-error)]'>
                {t('deploy.modal.undeploy.description')}
              </span>
            </p>
          </ModalBody>
          <ModalFooter>
            ...
            <Button variant='destructive' onClick={handleUndeploy} disabled={isUndeploying}>
              {isUndeploying ? t('deploy.modal.undeploy.undeploying') : t('deploy.modal.undeploy.button')}
            </Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
      ...
      <Modal open={showA2aDeleteConfirm} onOpenChange={setShowA2aDeleteConfirm}>
        <ModalContent size='sm'>
          <ModalHeader>{t('deploy.modal.a2a.deleteAgent')}</ModalHeader>
          <ModalBody>
            <p className='text-[12px] text-[var(--text-secondary)]'>
              {t('deploy.modal.a2a.deleteConfirm')}{' '}
              <span className='font-medium text-[var(--text-primary)]'>
                {existingA2aAgent?.name || t('deploy.modal.a2a.thisAgent')}
              </span>
              ?{' '}
              <span className='text-[var(--text-error)]'>
                {t('deploy.modal.a2a.deleteDescription')}
              </span>
            </p>
          </ModalBody>
          <ModalFooter>
            ...
            <Button variant='destructive' onClick={handleA2aDelete} disabled={a2aSubmitting}>
              {a2aSubmitting ? t('common.deleting') : t('common.delete')}
            </Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
...
function StatusBadge({ isWarning }: StatusBadgeProps) {
  const { t } = useTranslation()
  const label = isWarning ? t('deploy.updateDeployment') : t('common.live')
  return (
...
function TemplateStatusBadge({ status, views, stars }: TemplateStatusBadgeProps) {
  ...
  const label = isPending ? t('deploy.modal.a2a.underReview') : t('common.live')
      ...
      <ModalFooter className='items-center justify-between'>
        ...
        <div className='flex items-center gap-2'>
          <Button variant='tertiary' onClick={onDeploy} disabled={isSubmitting}>
            {isSubmitting ? t('deploy.deploying') : t('deploy.buttonDeploy')}
          </Button>
        </div>
      </ModalFooter>
    )
