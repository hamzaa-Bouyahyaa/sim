...
export const Panel = memo(function Panel() {
  ...
  return (
    <>
      <aside
        ref={panelRef}
        className='panel-container fixed inset-y-0 right-0 rtl:right-auto rtl:left-0 z-10 overflow-hidden bg-[var(--surface-1)]'
        aria-label={t('panel.workflowPanel')}
      >
        <div className='flex h-full flex-col border-[var(--border)] border-l rtl:border-l-0 rtl:border-r pt-[14px]'>
          {/* Header */}
          <div className='flex flex-shrink-0 items-center justify-between px-[8px]'>
              ...
              <Popover open={isMenuOpen} onOpenChange={setIsMenuOpen}>
                ...
                <PopoverContent align='start' side='bottom' sideOffset={8}>
                  <PopoverItem
                    ...
                    title={hasLockedBlocks ? t('panel.menu.unlockToAutoLayout') : undefined}
                  >
                    <Layout className='h-3 w-3' animate={isAutoLayouting} variant='clockwise' />
                    <span>{t('panel.menu.autoLayout')}</span>
                  </PopoverItem>
                  {
                    <PopoverItem onClick={() => setVariablesOpen(!isVariablesOpen)}>
                      <VariableIcon className='h-3 w-3' />
                      <span>{t('panel.menu.variables')}</span>
                    </PopoverItem>
                  }
                  {/* <PopoverItem>
                  ...
                  >
                    ...
                    <span>{t('panel.menu.exportWorkflow')}</span>
                  </PopoverItem>
                  <PopoverItem
                  ...
                  >
                    ...
                    <span>{t('panel.menu.duplicateWorkflow')}</span>
                  </PopoverItem>
                  <PopoverItem
                    onClick={() => {
                      setIsMenuOpen(false)
                  ...
                  >
                    ...
                    <span>{t('panel.menu.deleteWorkflow')}</span>
                  </PopoverItem>
                </PopoverContent>
              </Popover>
              <Button
            ...
            <div className='flex gap-[6px]'>
              ...
              >
                ...
                {isExecuting ? t('panel.stop') : t('panel.run')}
              </Button>
            </div>
          </div>
      ...
      {/* Resize Handle */}
      <div
        className='fixed top-0 right-[calc(var(--panel-width)-4px)] rtl:right-auto rtl:left-[calc(var(--panel-width)-4px)] bottom-0 z-20 w-[8px] cursor-ew-resize'
      ...
      <Modal open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
        <ModalContent size='sm'>
          <ModalHeader>{t('panel.deleteModal.title')}</ModalHeader>
          <ModalBody>
            <p className='text-[12px] text-[var(--text-secondary)]'>
              {t('panel.deleteModal.confirmText')}{' '}
              <span className='font-medium text-[var(--text-primary)]'>
                {currentWorkflow?.name ?? t('panel.deleteModal.thisWorkflow')}
              </span>
              ? {t('panel.deleteModal.permanentRemove')}{' '}
              <span className='text-[var(--text-error)]'>{t('panel.deleteModal.cannotBeUndone')}</span>
            </p>
          </ModalBody>
          <ModalFooter>
            <Button
