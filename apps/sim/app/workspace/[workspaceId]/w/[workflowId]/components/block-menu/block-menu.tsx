...
export function BlockMenu({
  ...
  const getToggleEnabledLabel = () => {
    if (allEnabled) return t('contextMenu.block.disable')
    if (allDisabled) return t('contextMenu.block.enable')
    return t('contextMenu.block.toggleEnabled')
  }

  const getToggleLockedLabel = () => {
    if (allLocked) return t('contextMenu.block.unlock')
    if (allUnlocked) return t('contextMenu.block.lock')
    return t('contextMenu.block.toggleLock')
  }
  ...
  return (
    ...
    >
      ...
      <PopoverContent ref={menuRef} align='start' side='bottom' sideOffset={4}>
        ...
        >
          <span>{t('contextMenu.block.copy')}</span>
        ...
        >
          <span>{t('contextMenu.block.paste')}</span>
        ...
        {!allNoteBlocks && (
          ...
          >
            {hasBlockWithDisabledParent ? t('contextMenu.block.parentDisabled') : getToggleEnabledLabel()}
          </PopoverItem>
        )}
        {!allNoteBlocks && !isSubflow && (
          ...
          >
            {t('contextMenu.block.flipHandles')}
          </PopoverItem>
        )}
        {canRemoveFromSubflow && (
          ...
          >
            {t('contextMenu.block.removeFromSubflow')}
          </PopoverItem>
        )}
        {canAdmin && onToggleLocked && (
          ...
          >
            {hasBlockWithLockedParent ? t('contextMenu.block.parentLocked') : getToggleLockedLabel()}
          </PopoverItem>
        )}
        ...
        {isSingleBlock && (
          ...
          >
            {t('contextMenu.block.openEditor')}
          </PopoverItem>
        )}
        ...
        {isSingleBlock && !allNoteBlocks && !isInsideSubflow && (
          <>
            ...
            >
              {t('contextMenu.block.runFromBlock')}
            </PopoverItem>
            ...
            {!hasTriggerBlock && (
              ...
              >
                {t('contextMenu.block.runUntilBlock')}
              </PopoverItem>
            )}
          </>
        )}
        ...
        >
          <span>{t('contextMenu.block.delete')}</span>
