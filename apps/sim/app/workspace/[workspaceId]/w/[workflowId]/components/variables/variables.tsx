...
/**
 * Placeholder values for code editors (not user-facing labels)
 */
const PLACEHOLDERS = {
  number: '42',
  boolean: 'true',
...
export function Variables() {
  ...
  const typeOptions: ComboboxOption[] = useMemo(
    () => [
      { label: t('variables.types.plain'), value: 'plain' },
      { label: t('variables.types.number'), value: 'number' },
      { label: t('variables.types.boolean'), value: 'boolean' },
      { label: t('variables.types.object'), value: 'object' },
      { label: t('variables.types.array'), value: 'array' },
    ],
    [t]
  )
  ...
  const handleVariableNameBlur = useCallback(
    (variableId: string) => {
      ...
      if (!trimmedName) {
        setNameErrors((prev) => ({
          ...prev,
          [variableId]: t('variables.nameEmpty'),
        }))
        return
      }
      ...
      if (isDuplicateName(variableId, trimmedName)) {
        setNameErrors((prev) => ({
          ...prev,
          [variableId]: t('variables.duplicateName'),
        }))
        return
      }
    ...
    [localNames, isDuplicateName, collaborativeUpdateVariable, t]
  )
      ...
      return (
        ...
        >
          ...
          >
            ...
            <span className='sr-only'>{t('variables.deleteVariable')}</span>
          </Button>
        </div>
      )
    },
    [collapsedById, toggleCollapsed, handleRemoveVariable, t]
  )
  ...
  const renderValueInput = useCallback(
    (variable: Variable) => {
      ...
      if (variable.type === 'object' || variable.type === 'array') {
        ...
        const placeholder =
          variable.type === 'object' ? PLACEHOLDERS.object : PLACEHOLDERS.array
      ...
      >
        ...
        <div className='flex items-center gap-[8px]'>
          <Button
            ...
            aria-label={t('variables.addVariable')}
          >
                  ...
                  {!(collapsedById[variable.id] ?? false) && (
                    ...
                    >
                      <div className='flex flex-col gap-[4px]'>
                        <Label className='text-[13px]'>{t('variables.nameLabel')}</Label>
                        <Input
                          name='name'
                          autoComplete='off'
                      ...
                      <div className='space-y-[4px]'>
                        <Label className='text-[13px]'>{t('variables.typeLabel')}</Label>
                        <Combobox
                          options={typeOptions}
                          value={variable.type}
                      ...
                      <div className='space-y-[4px]'>
                        <Label className='text-[13px]'>{t('variables.valueLabel')}</Label>
