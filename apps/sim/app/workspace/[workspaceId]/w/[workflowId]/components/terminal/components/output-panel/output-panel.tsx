...
export const OutputPanel = React.memo(function OutputPanel({
  ...
  return (
    <>
      <div
        className='absolute top-0 right-0 rtl:right-auto rtl:left-0 bottom-0 flex flex-col border-[var(--border)] border-l rtl:border-l-0 rtl:border-r bg-[var(--surface-1)]'
        style={{ width: `${outputPanelWidth}px` }}
      >
        {/* Horizontal Resize Handle */}
        <div
          className='-ml-[4px] rtl:ml-0 rtl:-mr-[4px] absolute top-0 bottom-0 left-0 rtl:left-auto rtl:right-0 z-20 w-[8px] cursor-ew-resize'
        ...
        >
          <div className='flex items-center'>
            <Button
              ...
              aria-label={t('terminal.output.output')}
            >
              {t('terminal.output.output')}
            </Button>
            {hasInputData && (
              <Button
                variant='ghost'
                ...
                aria-label={t('terminal.output.input')}
              >
                {t('terminal.output.input')}
              </Button>
            )}
          </div>
              ...
              <Tooltip.Root>
                <Tooltip.Trigger asChild>
                  <Button
                    ...
                    aria-label={t('terminal.output.closeSearch')}
                ...
                <Tooltip.Content>
                  <span>{t('terminal.output.closeSearch')}</span>
                </Tooltip.Content>
              </Tooltip.Root>
            ) : (
              <Tooltip.Root>
                <Tooltip.Trigger asChild>
                  <Button
                    ...
                    aria-label={t('terminal.output.search')}
                    className='!p-1.5 -m-1.5'
                  >
                ...
                <Tooltip.Content>
                  <span>{t('terminal.output.search')}</span>
                </Tooltip.Content>
              </Tooltip.Root>
            )}
              ...
              <Tooltip.Root>
                <Tooltip.Trigger asChild>
                  <Link href='/playground'>
                    <Button
                      variant='ghost'
                      aria-label={t('terminal.output.componentPlayground')}
                      className='!p-1.5 -m-1.5'
                    >
                ...
                <Tooltip.Content>
                  <span>{t('terminal.output.componentPlayground')}</span>
                </Tooltip.Content>
              </Tooltip.Root>
            )}
              ...
              <Tooltip.Root>
                <Tooltip.Trigger asChild>
                  <Button
                    ...
                    aria-label={isTraining ? t('terminal.output.stopTraining') : t('terminal.output.trainCopilot')}
                    className={clsx(
                      '!p-1.5 -m-1.5',
                ...
                <Tooltip.Content>
                  <span>{isTraining ? t('terminal.output.stopTraining') : t('terminal.output.trainCopilot')}</span>
                </Tooltip.Content>
              </Tooltip.Root>
            )}
                ...
                <Tooltip.Root>
                  <Tooltip.Trigger asChild>
                    <Button
                      ...
                      aria-label={t('terminal.downloadCsv')}
                      className='!p-1.5 -m-1.5'
                    >
                  ...
                  <Tooltip.Content>
                    <span>{t('terminal.downloadCsv')}</span>
                  </Tooltip.Content>
                </Tooltip.Root>
                <Tooltip.Root>
                  <Tooltip.Trigger asChild>
                    <Button
                      ...
                      aria-label={t('terminal.clearConsole')}
                      className='!p-1.5 -m-1.5'
                    >
                  ...
                  <Tooltip.Content>
                    <Tooltip.Shortcut keys='⌘D'>{t('terminal.clearConsole')}</Tooltip.Shortcut>
                  </Tooltip.Content>
                </Tooltip.Root>
              </>
            )}
            <Popover open={outputOptionsOpen} onOpenChange={setOutputOptionsOpen} size='sm'>
              <PopoverTrigger asChild>
                <Button
                  ...
                  aria-label={t('terminal.terminalOptions')}
                  className='!p-1.5 -m-1.5'
                >
              ...
              >
                ...
                >
                  <span>{t('terminal.output.structuredView')}</span>
                </PopoverItem>
                <PopoverItem active={wrapText} showCheck={wrapText} onClick={handleToggleWrapText}>
                  <span>{t('terminal.output.wrapText')}</span>
                </PopoverItem>
                <PopoverItem
                  active={openOnRun}
                  showCheck={openOnRun}
                ...
                >
                  <span>{t('terminal.openOnRun')}</span>
                </PopoverItem>
              </PopoverContent>
            </Popover>
        ...
        {isOutputSearchActive && (
          <div
            className='absolute top-[30px] right-[8px] rtl:right-auto rtl:left-[8px] z-30 flex h-[34px] items-center gap-[6px] rounded-b-[4px] border border-[var(--border)] border-t-0 bg-[var(--surface-1)] px-[6px] shadow-sm'
            onClick={(e) => e.stopPropagation()}
            data-toolbar-root
          ...
          >
            <Input
              ...
              placeholder={t('terminal.output.searchPlaceholder')}
            ...
            >
              {matchCount > 0 ? `${currentMatchIndex + 1}/${matchCount}` : t('common.noResults')}
            </span>
            <Button
              variant='ghost'
              onClick={goToPreviousMatch}
              aria-label={t('terminal.output.previousMatch')}
              className='!p-1.5 -m-1.5'
            ...
            <Button
              ...
              aria-label={t('terminal.output.nextMatch')}
              className='!p-1.5 -m-1.5'
              disabled={matchCount === 0}
            >
