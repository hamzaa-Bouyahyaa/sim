...
export function Editor() {
  ...
  const title = currentBlock?.name || t('editor.title')
          ...
          {isLocked && currentBlock && (
            <Tooltip.Root>
              <Tooltip.Trigger asChild>
                {userPermissions.canAdmin && currentBlock.locked && !isParentLocked ? (
                  <Button
                    ...
                    aria-label={t('editor.unlockBlock')}
                  >
              ...
              <Tooltip.Content side='top'>
                <p>
                  {isParentLocked
                    ? t('editor.parentLocked')
                    : userPermissions.canAdmin && currentBlock.locked
                      ? t('editor.unlockBlock')
                      : t('editor.blockLocked')}
                </p>
              </Tooltip.Content>
            </Tooltip.Root>
          )}
            ...
            <Tooltip.Root>
              <Tooltip.Trigger asChild>
                <Button
                  ...
                  aria-label={isRenaming ? t('editor.saveName') : t('editor.renameBlock')}
                >
                  {isRenaming ? (
              ...
              <Tooltip.Content side='top'>
                <p>{isRenaming ? t('editor.saveName') : t('editor.renameBlock')}</p>
              </Tooltip.Content>
            </Tooltip.Root>
          )}
                      ...
                      {isLoadingChildWorkflow ? (
                        ...
                        <>
                          ...
                          <Tooltip.Root>
                            ...
                            <Tooltip.Content side='top'>{t('editor.openWorkflow')}</Tooltip.Content>
                          </Tooltip.Root>
                        </>
                      ) : (
                        <div className='flex h-full items-center justify-center bg-[var(--surface-3)]'>
                          <span className='text-[13px] text-[var(--text-tertiary)]'>
                            {t('editor.unableToLoadPreview')}
                          </span>
                        </div>
                      )}
                    </div>
                ...
                <div className='flex flex-col'>
                  ...
                  {hasAdvancedOnlyFields && canEditBlock && (
                    <div className='flex items-center gap-[10px] px-[2px] pt-[14px] pb-[12px]'>
                      ...
                      >
                        {displayAdvancedOptions
                          ? t('editor.hideAdditionalFields')
                          : t('editor.showAdditionalFields')}
                        <ChevronDown
          ...
          {hasIncomingConnections && (
            ...
            >
              ...
              <div
                ...
                tabIndex={0}
                aria-label={
                  isConnectionsAtMinHeight ? t('editor.expandConnections') : t('editor.collapseConnections')
                }
              >
                <ChevronUp
                  className={
