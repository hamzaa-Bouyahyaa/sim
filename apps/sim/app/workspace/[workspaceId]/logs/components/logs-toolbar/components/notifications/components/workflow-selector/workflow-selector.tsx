...
export function WorkflowSelector({
  ...
  const overlayContent = useMemo(() => {
    if (allWorkflows) {
      return <span className='truncate text-[var(--text-primary)]'>{t('logs.toolbar.allWorkflows')}</span>
    }
  ...
  if (isLoading) {
    return (
      <div className='flex flex-col gap-[4px]'>
        <span className='font-medium text-[13px] text-[var(--text-secondary)]'>{t('logs.toolbar.workflowFilter')}</span>
  ...
  return (
    <div className='flex flex-col gap-[4px]'>
      ...
      <Combobox
        ...
        placeholder={t('logs.toolbar.searchWorkflows')}
        error={error}
