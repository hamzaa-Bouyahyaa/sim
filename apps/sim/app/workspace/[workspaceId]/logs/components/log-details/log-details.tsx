  ...
  function WorkflowOutputSection({ output }: { output: Record<string, unknown> }) {
    ...
    return (
      <div className='relative flex min-w-0 flex-col overflow-hidden'>
          ...
          {!isSearchActive && (
            <div className='absolute top-[7px] right-[6px] z-10 flex gap-[4px]'>
              <Tooltip.Root>
                ...
                <Tooltip.Content side='top'>{copied ? t('common.copied') : t('common.copy')}</Tooltip.Content>
              </Tooltip.Root>
              <Tooltip.Root>
                <Tooltip.Trigger asChild>
                  <Button
                ...
                <Tooltip.Content side='top'>{t('common.search')}</Tooltip.Content>
              </Tooltip.Root>
            </div>
          )}
        </div>
        ...
        {isSearchActive && (
          <div
            className='absolute top-0 right-0 rtl:right-auto rtl:left-0 z-30 flex h-[34px] items-center gap-[6px] rounded-[4px] border border-[var(--border)] bg-[var(--surface-1)] px-[6px] shadow-sm'
          ...
          >
            <Input
              ...
              placeholder={t('logs.details.searchPlaceholder')}
            ...
            <Button
              ...
              aria-label={t('logs.details.previousLog')}
            >
            ...
            <Button
              ...
              disabled={matchCount === 0}
              aria-label={t('logs.details.nextLog')}
            >
          ...
          createPortal(
            ...
            >
              ...
              <PopoverContent align='start' side='bottom' sideOffset={4}>
                <PopoverItem onClick={handleCopy}>{t('common.copy')}</PopoverItem>
                <PopoverDivider />
                <PopoverItem onClick={handleSearch}>{t('common.search')}</PopoverItem>
              </PopoverContent>
            </Popover>,
            document.body
          )}
    ...
    <>
      ...
      {isOpen && (
        <div
          ...
          role='separator'
          aria-label={t('logs.details.resizePanel')}
          aria-orientation='vertical'
        />
      )}

      <div
        className={`absolute top-[0px] right-0 rtl:right-auto rtl:left-0 bottom-0 z-50 transform overflow-hidden border-l rtl:border-l-0 rtl:border-r bg-[var(--surface-1)] shadow-md transition-transform duration-200 ease-out ${
        ...
        {log && (
          <div className='flex h-full flex-col px-[14px] pt-[12px]'>
            ...
            <div className='flex items-center justify-between'>
              <h2 className='font-medium text-[14px] text-[var(--text-primary)]'>{t('logs.details.title')}</h2>
              <div className='flex items-center gap-[1px]'>
                ...
                <Button variant='ghost' className='!p-[4px]' onClick={onClose} aria-label={t('common.close')}>
              ...
              <div className='flex flex-col gap-[10px] pb-[16px]'>
                  ...
                  <div className='flex w-[140px] flex-shrink-0 flex-col gap-[8px]'>
                    ...
                    <div className='flex items-center gap-[6px]'>
                      <span className='font-medium text-[14px] text-[var(--text-secondary)]'>
                        {formattedTimestamp?.compactDate || t('logs.details.na')}
                      </span>
                      <span className='font-medium text-[14px] text-[var(--text-secondary)]'>
                        {formattedTimestamp?.compactTime || t('logs.details.na')}
                      </span>
                    </div>
                  </div>
                ...
                {isWorkflowExecutionLog && log.executionId && !permissionConfig.hideTraceSpans && (
                  <div className='-mt-[8px] flex flex-col gap-[6px] rounded-[6px] border border-[var(--border)] bg-[var(--surface-2)] px-[10px] py-[8px]'>
                    ...
                    >
                      <span className='font-medium text-[12px]'>{t('logs.details.viewSnapshot')}</span>
                  ...
                  <div className='flex flex-col gap-[8px]'>
                    ...
                    <div className='flex items-center justify-center rounded-[6px] bg-[var(--surface-2)] p-[8px] text-center'>
                      <p className='font-medium text-[11px] text-[var(--text-subtle)]'>
                        {t('logs.details.totalCostDescription')}{' '}
                        {formatCost(BASE_EXECUTION_CHARGE)} {t('logs.details.plusModelCosts')}
                      </p>
                    </div>
                  </div>
                )}
