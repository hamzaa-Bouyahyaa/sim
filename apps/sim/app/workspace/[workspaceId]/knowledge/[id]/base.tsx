...
function KnowledgeBaseLoading({ knowledgeBaseName }: KnowledgeBaseLoadingProps) {
  ...
  const breadcrumbItems = [
    { label: t('knowledge.title'), href: `/workspace/${workspaceId}/knowledge` },
          ...
          <div className='mt-[14px] flex items-center justify-between'>
            ...
            <div className='flex items-center gap-[8px]'>
              ...
              <Button disabled variant='tertiary' className='h-[32px] rounded-[6px]'>
                {t('knowledge.documents.addDocuments')}
              </Button>
            </div>
          </div>
    ...
    <div className='flex h-full flex-1 flex-col'>
            ...
            <div className='flex items-center gap-[8px]'>
              ...
              <Tooltip.Root>
                ...
                {userPermissions.canEdit !== true && (
                  <Tooltip.Content>{t('knowledge.writePermRequired')}</Tooltip.Content>
                )}
              </Tooltip.Root>
            </div>
          </div>
                  ...
                  <p className='mt-1 text-[var(--text-muted)] text-xs'>
                    {searchQuery
                      ...
                      : enabledFilter !== 'all'
                        ? t('knowledge.tryDifferentSearch')
                        : userPermissions.canEdit === true
                          ? t('knowledge.getStarted')
                          : t('knowledge.willAppearHere')}
                  </p>
                </div>
              </div>
            ) : (
                      ...
                      >
                        ...
                        <TableCell className='px-[12px] py-[8px]'>
                          {(() => {
                            ...
                            const displayText = tags.map((tag) => tag.value).join(', ')
                            return (
                              <Tooltip.Root>
                                <Tooltip.Trigger asChild>
                                  <span
                          ...
                          <div className='flex items-center gap-[4px]'>
                            {doc.processingStatus === 'failed' && (
                              <Tooltip.Root>
                                ...
                                <Tooltip.Content side='top'>{t('common.run')}</Tooltip.Content>
                              </Tooltip.Root>
                            )}
                            <Tooltip.Root>
                              <Tooltip.Trigger asChild>
        ...
        <ModalContent size='sm'>
          ...
          <ModalFooter>
            ...
            <Button variant='destructive' onClick={handleDeleteKnowledgeBase} disabled={isDeleting}>
              {isDeleting ? t('common.deleting') : t('knowledge.deleteKnowledgeBase')}
            </Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
          ...
          <ModalBody>
            <p className='text-[12px] text-[var(--text-secondary)]'>
              ...
              <span className='font-medium text-[var(--text-primary)]'>
                {documents.find((doc) => doc.id === documentToDelete)?.filename ?? ''}
              </span>
              ? <span className='text-[var(--text-error)]'>{t('common.cannotBeUndone')}</span>
            </p>
          </ModalBody>
      ...
      <Modal open={showBulkDeleteModal} onOpenChange={setShowBulkDeleteModal}>
        <ModalContent size='sm'>
          ...
          <ModalBody>
            <p className='text-[12px] text-[var(--text-secondary)]'>
              {t('knowledge.deleteModal.confirmText')} {selectedDocuments.size}?{' '}
              <span className='text-[var(--text-error)]'>{t('common.cannotBeUndone')}</span>
            </p>
          </ModalBody>
          <ModalFooter>
