      ...
      <ModalContent size='sm'>
        <ModalHeader>
          <div className='flex items-center justify-between'>
            <span>{t('knowledge.tags.documentTags')}</span>
          </div>
        </ModalHeader>
        ...
        <ModalBody>
          <div className='min-h-0 flex-1 overflow-y-auto'>
            <div className='space-y-[8px]'>
              <Label>{t('knowledge.tags.tags')}</Label>
                  ...
                  {editingTagIndex === index && (
                    <div className='space-y-[8px] rounded-[6px] border p-[12px]'>
                      <div className='flex flex-col gap-[8px]'>
                        <Label htmlFor={`tagName-${index}`}>{t('knowledge.tags.tagName')}</Label>
                        {availableDefinitions.length > 0 ? (
                          <Combobox
                            ...
                            placeholder={t('knowledge.tags.tagNamePlaceholder')}
                            editable={true}
                          ...
                          <Input
                            ...
                            placeholder={t('knowledge.tags.enterTagName')}
                            className={cn(tagNameConflict && 'border-[var(--text-error)]')}
                            onKeyDown={(e) => {
                      ...
                      <div className='flex flex-col gap-[8px]'>
                        <Label htmlFor={`tagValue-${index}`}>{t('common.value')}</Label>
                        {editTagForm.fieldType === 'boolean' ? (
                          <Combobox
                            ...
                            placeholder={t('knowledge.tags.selectValue')}
                          />
                          ...
                          <Input
                            ...
                            placeholder={t('knowledge.tags.enterNumber')}
                            inputMode='decimal'
                            onKeyDown={(e) => {
                              if (e.key === 'Enter' && canSaveTag) {
                                e.preventDefault()
                          ...
                          <DatePicker
                            ...
                            onChange={(value) => setEditTagForm({ ...editTagForm, value })}
                            placeholder={t('knowledge.tags.selectDate')}
                          />
                        ) : (
                          <Input
                            id={`tagValue-${index}`}
                            ...
                            placeholder={t('knowledge.tags.enterTagValue')}
                            onKeyDown={(e) => {
                              if (e.key === 'Enter' && canSaveTag) {
                                e.preventDefault()
                                saveDocumentTag()
              ...
              {documentTags.length > 0 && !isTagEditing && (
                <Button
                  ...
                  className='w-full'
                >
                  {t('common.add')} {t('knowledge.tags.tags')}
                </Button>
              )}
              ...
              {(isCreatingTag || documentTags.length === 0) && editingTagIndex === null && (
                <div className='space-y-[8px] rounded-[6px] border p-[12px]'>
                  <div className='flex flex-col gap-[8px]'>
                    <Label htmlFor='newTagName'>{t('knowledge.tags.tagName')}</Label>
                    ...
                    {tagNameConflict && (
                      <span className='text-[12px] text-[var(--text-error)]'>
                        {t('knowledge.tags.tagName')}
                      </span>
                    )}
                  </div>

                  <div className='flex flex-col gap-[8px]'>
                    <Label htmlFor='newTagValue'>{t('common.value')}</Label>
                    {editTagForm.fieldType === 'boolean' ? (
                      <Combobox
                        id='newTagValue'
                        options={[
                  ...
                  <div className='flex gap-[8px]'>
                    ...
                    >
                      {isSavingTag ? t('common.creating') : t('common.create')}
                    </Button>
                  </div>
                </div>
              )}
