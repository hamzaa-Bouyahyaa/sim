  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange}>
      <ModalContent size='sm'>
        <ModalHeader>{t('knowledge.documents.renameDocument')}</ModalHeader>
        <form onSubmit={handleSubmit} className='flex min-h-0 flex-1 flex-col'>
          <ModalBody className='!pb-[16px]'>
            <div className='space-y-[12px]'>
              <div className='flex flex-col gap-[8px]'>
                <Label htmlFor='document-name'>{t('common.name')}</Label>
                <Input
                  id='document-name'
                  value={name}
                  onChange={(e) => {
                  ...
                  placeholder={t('knowledge.documents.namePlaceholder')}
          ...
          <ModalFooter>
            <div className='flex w-full items-center justify-between gap-[12px]'>
              ...
              <div className='flex flex-shrink-0 gap-[8px]'>
                ...
                >
                  {isSubmitting ? t('knowledge.documents.renaming') : t('common.rename')}
                </Button>
              </div>
            </div>
          </ModalFooter>
