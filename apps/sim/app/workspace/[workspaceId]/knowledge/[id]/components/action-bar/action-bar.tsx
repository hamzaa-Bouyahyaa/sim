    ...
    >
      <div className='flex items-center gap-[8px] rounded-[10px] border border-[var(--border)] bg-[var(--surface-2)] px-[8px] py-[6px]'>
        <span className='px-[4px] text-[13px] text-[var(--text-secondary)]'>
          {isAllSelected ? totalCount : selectedCount} {t('common.selectAll').split(' ').pop()}
          {showSelectAllOption && (
            <>
              {' · '}
              <button
        ...
        <div className='flex items-center gap-[5px]'>
          {showEnableButton && (
            <Tooltip.Root>
              ...
              <Tooltip.Content side='top'>{t('common.enable')}</Tooltip.Content>
            </Tooltip.Root>
          )}
          ...
          {showDisableButton && (
            <Tooltip.Root>
              ...
              <Tooltip.Content side='top'>{t('common.disable')}</Tooltip.Content>
            </Tooltip.Root>
          )}
          ...
          {onDelete && canEdit && (
            <Tooltip.Root>
              ...
              <Tooltip.Content side='top'>{t('common.delete')}</Tooltip.Content>
            </Tooltip.Root>
          )}
        </div>
      </div>
